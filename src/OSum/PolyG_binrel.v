...
From MyProject.src.OSum Require Export PolyG OSum persistent_pred wp_rules binary_rules.
...
Module PolyGBinRel.
    ...
    Notation Gtype := MyProject.src.OSum.PolyG.PolyG.type.
...
Section binary_rel.
    ...
    Program Definition interp_TExistNew (interp : VRel) : VRel :=
        λne Δ,PersPred
            (λ ww, ∃ vv (R : D) , ⌜ww = (PackV vv.1, PackV vv.2)⌝ ∗ 
            □  ∀ (sig1 sig2 : loc), 
            |==> sig1 @ sig2 ↦□ R →
                    interp_expr interp (R :: Δ) 
                    ((App (of_val vv.1) (Case sig1)), (App (of_val vv.2) (Case sig2))))%I.
    Solve Obligations with repeat intros ?; simpl; solve_proper.

    Program Definition interp_TForallNew (interp : VRel) : VRel := 
        λne Δ, PersPred(fun ww =>
        □ ∀ (R : D) (sig1 sig2 : loc),
            |==> sig1 @ sig2 ↦□ R →
            interp_expr
            interp (R :: Δ) (App (TApp (of_val (ww.1))) (Case sig1), (App (TApp (of_val (ww.2))) (Case sig2))))%I.
    Solve Obligations with solve_proper.
    ...
    Fixpoint interp (τ : Gtype) : VRel :=
        match τ return _ with
            ...
            | PolyG.TProd t1 t2 => interp_TProd (interp t1) (interp t2)
            | PolyG.TArrow t1 t2 => interp_TArrow (interp t1) (interp t2)
            | PolyG.TExistNew t => interp_TForallNew (interp t)
            | PolyG.TForallNew t => interp_TExistNew (interp t)
        end.
